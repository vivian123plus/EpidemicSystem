...
// pointer-event:none  穿透画布，保证能够和页面元素正常互动  filter：blur设置模糊，多少像素融合
canvas.setAttribute('style', 'position:fixed;left:0;top:0;pointer-events:none;filter:blur(1px);')
var clicks = []
var points = [] //定义粒子数组
var live = 50 //存活50个周期
var colors = [  //备选粒子颜色数组
...
function drawpoints() { //绘制粒子
    ...
    for (var i = 0; i < points.length; i++) { //遍历粒子
        ...
        ctx.fillStyle = "rgba(" + point.color + "," + point.life / live + ")" //根据粒子属性设置填充颜色及透明度,life会慢慢变小
        ctx.fill() //填充颜色
        point.life-- //生命值减1
